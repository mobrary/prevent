...
public class DBSQLSERVER extends JDBBASE implements IDB {
    private final static Logger log = Logger.getLogger(DBSQLSERVER.class);
    ...
    public List<Map<String, Object>> QueryList(String cFileList, String cTableName, String cWhereParm, String cOrderBy, int iPageNo, int iPageSize) {
        ...
        sql.append("    select " + cFileList + ", ROW_NUMBER() OVER(Order by " + cOrderBy + " ) AS ROW_ID ");
        sql.append("    from " + cTableName);
        if (!StringUtils.isEmpty(cWhereParm)) {
            sql.append(" WHERE  " + cWhereParm);
        }
        sql.append(" ) as tab ");
        sql.append(" where ROW_ID between " + iStart + " and " + iFinish);
