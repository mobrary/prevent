...
import com.tlkj.gw.entity.XT_INTERFACE;
import org.apache.commons.lang3.StringUtils;
import com.tlkj.framework.BeanUtil;

public class XT_INTERFACE_Dao implements IService<XT_INTERFACE> {

	public Integer Save(XT_INTERFACE vo, String cKeyID) {
        String sql;
        if (StringUtils.isEmpty(cKeyID))
            sql = BeanUtil.getInsertSQL(vo, "XT_INTERFACE");
        else
            sql = BeanUtil.getUpdateSQL(vo, "XT_INTERFACE", "interface_url='" + cKeyID + "'");
        return DbManager.ExeSql(sql.toString());
    }

    public Integer DeleteItem(String cKeyID) {
		StringBuilder sql = new StringBuilder();
        sql.append(" DELETE FROM XT_INTERFACE ");
		sql.append(" WHERE interface_url='"+cKeyID+"'"); 
		return DbManager.ExeSql(sql.toString());
    }
    ...
    public Integer DeleteList(String cKeyList) {
		...
		StringBuilder sql = new StringBuilder();
		sql.append(" DELETE FROM XT_INTERFACE ");
		sql.append(" WHERE interface_url IN (");
    ...
    public XT_INTERFACE FindItem(String cKeyID) { 
		String cWhereParm = " interface_url='" + cKeyID + "'";
        List<XT_INTERFACE> rs = getList(cWhereParm);
        return (rs != null && rs.size() > 0) ? rs.get(0) : null;
    }

    public List<XT_INTERFACE> getList(String cWhereParm) {
       String sql = "SELECT * FROM XT_INTERFACE ";
	   if (!StringUtils.isEmpty(cWhereParm)) {
            sql = sql + " WHERE " + cWhereParm;
        }
        List<XT_INTERFACE> KeyList = new ArrayList<XT_INTERFACE>();
        List<Map<String, Object>> rs = DbManager.QueryList(sql);
        for (int i = 0; (rs != null) && (i < rs.size()); i++) {
            XT_INTERFACE vo = new XT_INTERFACE();
            Map<String, Object> rowKey = rs.get(i); 
            vo = (XT_INTERFACE) BeanUtil.Map2Obj(rowKey, vo);            
            KeyList.add(vo);
        }
        return KeyList;
    }
    ...
    public DBResult Query(String cFileList, String cTableName, String cWhereParm, String cOrderBy, int iPageNo, int iPageSize) {
        return DbManager.Query(cFileList, "XT_INTERFACE", cWhereParm, cOrderBy, iPageNo, iPageSize);
    }
    ...
    public List<Map<String, Object>> getListAsMap(String cFileList, String cWhereParm, String cOrderBy) {
         return DbManager.QueryList(cFileList,"XT_INTERFACE", cWhereParm, cOrderBy);
    }

    public List<Map<String, Object>> getListAsMap( String cWhereParm, String cOrderBy) {
         return DbManager.QueryList("*","XT_INTERFACE", cWhereParm, cOrderBy);
    }

    public List<Map<String, Object>> getListAsMap(String cWhereParm) {
         return DbManager.QueryList("*","XT_INTERFACE", cWhereParm,null);
    }
    ...
    public List<Map<String, Object>> getListAsMap(String cFileList, String cTableName, String cWhereParm, String cOrderBy, int iPageNo, int iPageSize) {
         return DbManager.QueryList(cFileList,"XT_INTERFACE", cWhereParm, cOrderBy, iPageNo, iPageSize);
    }
}
