...
import com.tlkj.gw.entity.XT_PHONE_BOOK;
import org.apache.commons.lang3.StringUtils;
import com.tlkj.framework.BeanUtil;

public class XT_PHONE_BOOK_Dao implements IService<XT_PHONE_BOOK> {

	public Integer Save(XT_PHONE_BOOK vo, String cKeyID) {
        String sql;
        if (StringUtils.isEmpty(cKeyID))
            sql = BeanUtil.getInsertSQL(vo, "XT_PHONE_BOOK");
        else
            sql = BeanUtil.getUpdateSQL(vo, "XT_PHONE_BOOK", "id='" + cKeyID + "'");
        return DbManager.ExeSql(sql.toString());
    }

    public Integer DeleteItem(String cKeyID) {
		StringBuilder sql = new StringBuilder();
        sql.append(" DELETE FROM XT_PHONE_BOOK ");
    ...
    public XT_PHONE_BOOK FindItem(String cKeyID) { 
		String cWhereParm = " id='" + cKeyID + "'";
        List<XT_PHONE_BOOK> rs = getList(cWhereParm);
        return (rs != null && rs.size() > 0) ? rs.get(0) : null;
    }

    public List<XT_PHONE_BOOK> getList(String cWhereParm) {
       String sql = "SELECT * FROM XT_PHONE_BOOK ";
	   if (!StringUtils.isEmpty(cWhereParm)) {
            sql = sql + " WHERE " + cWhereParm;
        }
        List<XT_PHONE_BOOK> KeyList = new ArrayList<XT_PHONE_BOOK>();
        List<Map<String, Object>> rs = DbManager.QueryList(sql);
        for (int i = 0; (rs != null) && (i < rs.size()); i++) {
            XT_PHONE_BOOK vo = new XT_PHONE_BOOK();
            Map<String, Object> rowKey = rs.get(i); 
            vo = (XT_PHONE_BOOK) BeanUtil.Map2Obj(rowKey, vo);            
            KeyList.add(vo);
        }
        return KeyList;
    }
    ...
    public DBResult Query(String cFileList, String cTableName, String cWhereParm, String cOrderBy, int iPageNo, int iPageSize) {
        return DbManager.Query(cFileList, "XT_PHONE_BOOK", cWhereParm, cOrderBy, iPageNo, iPageSize);
    }
    ...
    public List<Map<String, Object>> getListAsMap(String cFileList, String cWhereParm, String cOrderBy) {
         return DbManager.QueryList(cFileList,"XT_PHONE_BOOK", cWhereParm, cOrderBy);
    }

    public List<Map<String, Object>> getListAsMap( String cWhereParm, String cOrderBy) {
         return DbManager.QueryList("*","XT_PHONE_BOOK", cWhereParm, cOrderBy);
    }

    public List<Map<String, Object>> getListAsMap(String cWhereParm) {
         return DbManager.QueryList("*","XT_PHONE_BOOK", cWhereParm,null);
    }
    ...
    public List<Map<String, Object>> getListAsMap(String cFileList, String cTableName, String cWhereParm, String cOrderBy, int iPageNo, int iPageSize) {
         return DbManager.QueryList(cFileList,"XT_PHONE_BOOK", cWhereParm, cOrderBy, iPageNo, iPageSize);
    }
}
