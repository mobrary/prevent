...
public class XT_WF_ACTION_Dao implements IService<XT_WF_ACTION> {

	public Integer Save(XT_WF_ACTION vo, String cKeyID) {
        String sql;
        if (StringUtils.isEmpty(cKeyID))
            sql = BeanUtil.getInsertSQL(vo, "XT_WF_ACTION");
        else
            sql = BeanUtil.getUpdateSQL(vo, "XT_WF_ACTION", "action_id='" + cKeyID + "'");
        return DbManager.ExeSql(sql.toString());
    }

    public Integer DeleteItem(String cKeyID) {
		StringBuilder sql = new StringBuilder();
        sql.append(" DELETE FROM XT_WF_ACTION ");
    ...
    public XT_WF_ACTION FindItem(String cKeyID) { 
		String cWhereParm = " action_id='" + cKeyID + "'";
        List<XT_WF_ACTION> rs = getList(cWhereParm);
        return (rs != null && rs.size() > 0) ? rs.get(0) : null;
    }

    public List<XT_WF_ACTION> getList(String cWhereParm) {
       String sql = "SELECT * FROM XT_WF_ACTION ";
	   if (!StringUtils.isEmpty(cWhereParm)) {
            sql = sql + " WHERE " + cWhereParm;
        }
        List<XT_WF_ACTION> KeyList = new ArrayList<XT_WF_ACTION>();
        List<Map<String, Object>> rs = DbManager.QueryList(sql);
        for (int i = 0; (rs != null) && (i < rs.size()); i++) {
            XT_WF_ACTION vo = new XT_WF_ACTION();
            Map<String, Object> rowKey = rs.get(i); 
            vo = (XT_WF_ACTION) BeanUtil.Map2Obj(rowKey, vo);            
            KeyList.add(vo);
        }
        return KeyList;
    }
    ...
    public DBResult Query(String cFileList, String cTableName, String cWhereParm, String cOrderBy, int iPageNo, int iPageSize) {
        return DbManager.Query(cFileList, "XT_WF_ACTION", cWhereParm, cOrderBy, iPageNo, iPageSize);
    }
    ...
    public List<Map<String, Object>> getListAsMap(String cFileList, String cWhereParm, String cOrderBy) {
         return DbManager.QueryList(cFileList,"XT_WF_ACTION", cWhereParm, cOrderBy);
    }

    public List<Map<String, Object>> getListAsMap( String cWhereParm, String cOrderBy) {
         return DbManager.QueryList("*","XT_WF_ACTION", cWhereParm, cOrderBy);
    }

    public List<Map<String, Object>> getListAsMap(String cWhereParm) {
         return DbManager.QueryList("*","XT_WF_ACTION", cWhereParm,null);
    }
    ...
    public List<Map<String, Object>> getListAsMap(String cFileList, String cTableName, String cWhereParm, String cOrderBy, int iPageNo, int iPageSize) {
         return DbManager.QueryList(cFileList,"XT_WF_ACTION", cWhereParm, cOrderBy, iPageNo, iPageSize);
    }
}
