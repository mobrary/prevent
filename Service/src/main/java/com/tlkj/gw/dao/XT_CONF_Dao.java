...
public class XT_CONF_Dao implements IService<XT_CONF> {

	public Integer Save(XT_CONF vo, String cKeyID) {
        String sql;
        if (StringUtils.isEmpty(cKeyID))
            sql = BeanUtil.getInsertSQL(vo, "XT_CONF");
        else
            sql = BeanUtil.getUpdateSQL(vo, "XT_CONF", "conf_id='" + cKeyID + "'");
        return DbManager.ExeSql(sql.toString());
    }

    public Integer DeleteItem(String cKeyID) {
		StringBuilder sql = new StringBuilder();
        sql.append(" DELETE FROM XT_CONF ");
    ...
    public XT_CONF FindItem(String cKeyID) { 
		String cWhereParm = " conf_id='" + cKeyID + "'";
        List<XT_CONF> rs = getList(cWhereParm);
        return (rs != null && rs.size() > 0) ? rs.get(0) : null;
    }

    public List<XT_CONF> getList(String cWhereParm) {
       String sql = "SELECT * FROM XT_CONF ";
	   if (!StringUtils.isEmpty(cWhereParm)) {
            sql = sql + " WHERE " + cWhereParm;
        }
        List<XT_CONF> KeyList = new ArrayList<XT_CONF>();
        ...
        for (int i = 0; (rs != null) && (i < rs.size()); i++) {
            ...
            vo = (XT_CONF) BeanUtil.Map2Obj(rowKey, vo);            
            KeyList.add(vo);
        }
        return KeyList;
    }
    ...
    public DBResult Query(String cFileList, String cTableName, String cWhereParm, String cOrderBy, int iPageNo, int iPageSize) {
        return DbManager.Query(cFileList, "XT_CONF", cWhereParm, cOrderBy, iPageNo, iPageSize);
    }
    ...
    public List<Map<String, Object>> getListAsMap(String cFileList, String cWhereParm, String cOrderBy) {
         return DbManager.QueryList(cFileList,"XT_CONF", cWhereParm, cOrderBy);
    }

    public List<Map<String, Object>> getListAsMap( String cWhereParm, String cOrderBy) {
         return DbManager.QueryList("*","XT_CONF", cWhereParm, cOrderBy);
    }

    public List<Map<String, Object>> getListAsMap(String cWhereParm) {
         return DbManager.QueryList("*","XT_CONF", cWhereParm,null);
    }
    ...
    public List<Map<String, Object>> getListAsMap(String cFileList, String cTableName, String cWhereParm, String cOrderBy, int iPageNo, int iPageSize) {
         return DbManager.QueryList(cFileList,"XT_CONF", cWhereParm, cOrderBy, iPageNo, iPageSize);
    }
}
